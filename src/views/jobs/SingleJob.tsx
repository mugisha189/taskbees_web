...
const JobDetail = () => {
  ...
  const {mutate:applyForJob,isPending:applicationPending}=useApplyForJob(job?.id)
        ...
        <div className="flex md:flex-row flex-col justify-between md:items-center items-start">
          ...
          <div className="md:text-right text-start md:w-fit w-full">
            <p className="text-red-500 font-semibold">
              Application ends: <span className="text-black">{job&&job.application_deadline&&formatDateReadable(job.application_deadline)}</span>
            </p>
            <button onClick={handleApplyForJob} disabled={applicationPending} className="bg-yellow-400 md:w-fit w-full transition-all duration-100 hover:scale-[1.02] text-white font-semibold px-6 py-2 mt-2 rounded-lg hover:bg-yellow-500">
              {applicationPending ? <div className="w-full flex items-center justify-center">
                <Spinner />
              </div>:"Apply Now"}
            </button>
          </div>
      ...
      <div className="grid border rounded-lg grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white  rounded-lg  p-6">
          ...
          <div className="space-y-4">
            <p>
              <span className="font-semibold">Date Posted:</span> { job&&job.updatedAt&&formatDateReadable(job.updatedAt) }
            </p>
            <p>
              <span className="font-semibold">Location:</span> {job&&job.location}
            </p>
            <p>
              <span className="font-semibold">Expiration Date:</span> 
              {job&&job.application_deadline&&formatDateReadable(job.application_deadline)}
            </p>
            <p>
              <span className="font-semibold">Experience:</span> {job?.experience&&formatOptionValue(job.experience)}
            </p>
            {/* <p>
