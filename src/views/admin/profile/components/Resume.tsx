...
function Resume({
  ...
  const [existingResume, setExistingResume] = useState(user?.profile.resume);
  ...
  return (
    <div className="space-y-4">
      ...
      {file ? (
        ...
        <>
          <p className="text-base font-semibold mb-2">Existing Resume</p>
          <div className="border p-3 rounded-md w-fit">
            <p className="text-sm truncate">
              {user?.profile.fullnames}'s Resume
            </p>
            <p className="text-xs truncate ">
              {user?.profile.resume.slice(0, 50)}
            </p>
          </div>
        </>
      ) : null}
