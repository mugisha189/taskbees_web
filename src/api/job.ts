...
export const acceptCandidateApplication = async (
  jobId: string | undefined,
  candidateId: string | undefined
) => {
  try {
    const response = await PROTECTED_API.patch(
      `/business/business/applications/accept`,
      {
...
export const rejectCandidateApplication = async (
  jobId: string | undefined,
  candidateId: string | undefined
) => {
  try {
    const response = await PROTECTED_API.patch(
      `/business/business/applications/reject`,
      {
        applicantId: candidateId,
        jobId: jobId,
      }
...
export const approveForWork = async (
  jobId: string | undefined,
  candidateId: string | undefined
) => {
  try {
    const response = await PROTECTED_API.patch(
      `/business/business/applications/approve`,
      {
        applicantId: candidateId,
        jobId: jobId,
      }
...
export const createInterview = async (data: InterviewRequestPayload) => {
  try {
    ...
    const response = await PROTECTED_API.post(
      `/business/sync/interviews/${interviewId}`,
      { userIds: [data.applicantId] }
    );
    return response.data.payload;
  } catch (error) {
    if (error instanceof AxiosError) {
...
export const signAgreement = async (data: SignAgreementPayload) => {
  try {
    const { name, signature, jobId } = data;
    ...
    await PROTECTED_API.post(`/agreement/sign-agreement/${jobId}`, formData, {
